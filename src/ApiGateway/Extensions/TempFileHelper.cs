...
public static class TempFileHelper
{
    ...
    {
        if (!string.IsNullOrEmpty(_ocelotTempDir))
            return _ocelotTempDir;
        var tempPath = Path.Combine(Path.GetTempPath(),
            Path.GetRandomFileName());
        Directory.CreateDirectory(tempPath); // Ensure the directory is created
        _ocelotTempDir = Path.Combine(tempPath,
            ConfigConstants.OcelotConfigFile);
        return _ocelotTempDir;
    }
    ...
    public static string GetLiteDbTempDir()
    {
        ...
        _liteDbTempDir = Path.Combine(tempPath,
            MocksConstants.FileDbName);
        return _liteDbTempDir;
    }
}